...
# Cutoff date - filter out past events (use current date)
...
def main():
    ...
    # Dynamically find files by pattern (supports any date)
    # Look for most recent files for each site
    viva_files = sorted(HTML_DIR.glob("*-viva-*.html"), reverse=True)
    more_files = sorted(HTML_DIR.glob("*-more-*.html"), reverse=True)
    gazarte_files = sorted(HTML_DIR.glob("*-gazarte-*.html"), reverse=True)

    # Get today's files first, then fall back to most recent
    from datetime import datetime
    today = datetime.now().strftime('%Y-%m-%d')

    def get_latest_files(files, prefix):
        ...
        if files:
            # Extract date from first file and get all files from that date
            first_date = files[0].name.split('-')[0:3]  # ['2025', '12', '04']
            date_prefix = '-'.join(first_date)
            return [f for f in files if f.name.startswith(date_prefix)]
        return []

    files_to_parse = []
    files_to_parse.extend([(f, 'viva') for f in get_latest_files(viva_files, 'viva')])
    files_to_parse.extend([(f, 'more') for f in get_latest_files(more_files, 'more')])
    files_to_parse.extend([(f, 'gazarte') for f in get_latest_files(gazarte_files, 'gazarte')])

    if not files_to_parse:
        print("⚠️  No HTML files found in", HTML_DIR)
        print("   Run: python3 scripts/scrape-all-sites.py first")
        return

    print(f"📄 Found {len(files_to_parse)} HTML files to parse\n")

    for file_path, site_type in files_to_parse:
