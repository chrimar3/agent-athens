/**
 * The one way scripts write events URL columns (url, ticket_url,
 ...
 * that is not a safe URL is written as NULL, the same rule upsertEvent()
 * applies in src/db/database.ts. Values bound in a WHERE clause are left as
 * given, so lookups still match the stored text.
 *
 * tests/security/url-column-writes.test.ts fails on any SQL in scripts/*.ts
 * that assigns a URL column from a bound value without going through this
 * helper. Assigning NULL, or a column to itself, needs no helper.
 */
import type { Database, Statement } from 'bun:sqlite';
import { safeHttpUrl, safeImageSrc } from '../../src/utils/safe-url';
