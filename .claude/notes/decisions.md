...
**Ship:** S143 emitter + `LOCATION_NOT_INLINE` rule + spec `§2.4 v2` + manifest Surface 1 entry + tests in `369dfe905`. **Canonical-node geo-check (the GEO-aligned validator behavior described above) ships in `292e97aee`** — `369dfe905` shipped a green-build stopgap (demote `location.geo` WARN→INFO); `292e97aee` superseded it with the `@id`-lookup scope-to-canonical implementation that surfaces the routable signal (25 actionable WARN gaps → Component-B backfill). Companion fix S138 streetAddress at `b0b24fb64` (canonical-venue address backfill — config + migration script + DB upsert, 16 rows).
...
## S159 — Demo-hardening fixes: colophon wiring, filter mask, search reveal (2026-05-26)
...
1. **Colophon wired via shared chrome only (Option A), `page.ts` untouched.** `renderColophonTrigger/Dialog/Script` wired into `site-chrome.ts` (`renderSiteNav` emits trigger+dialog; `renderHamburgerScript` appends the colophon IIFE) → reaches every nav-bearing page with zero per-template edits and zero changes to `page.ts`'s locale-aware OG. Trigger placed **left of the search button** (Christos's explicit call); the existing `colophon.test.ts` ordering assertion was flipped to match (it had asserted right-of-search). Label shortened "About me" → **"About"** on the visible button + noscript only; aria-label, dialog sr-only heading, and the `generate-site.ts` standalone-page title/JSON-LD name kept "About me" (do-not-touch + fuller accessible name is correct AX).

2. **Filter edge-fade mask suppressed while a panel is open.** `.filter-bar-scroll` carries a horizontal `mask-image` fade for the pill scroller; it clips the dropdown (which overflows below the bar) even with `overflow:visible`. The existing `.filter-bar.has-open-panel .filter-bar-scroll` lift now also sets `mask-image:none; -webkit-mask-image:none`. The fade is cosmetic and only meaningful while scrolling pills — invisible to suppress while a panel is open. Minimal 2-line add to the existing rule; no color (accent budget unchanged); collaborator colophon-CSS WIP in the same file untouched.

3. **Search group reveal is explicit `'block'`, not `''`.** Because `.search-group { display:none }` is the stylesheet default, the reveal must set an explicit `display:'block'` (3 sites in `search-overlay.ts`) — `''` reverts to the hidden default. Rule: force-show is an explicit value, never empty-string.
...
**Production build path = `scripts/daily-automated.sh`** (2026-05-29). Raw `bun run build` is dev/debug, not deploy-grade. The daily-automated wrapper runs the Athens location filter, geocoding, and backfill before invoking `bun run build`, then `netlify deploy --prod --no-build --dir=dist`. Verified: today's 08:30 daily run deployed `6a192498a34cb2c1a1c12297` (state=ready, error=none, cli_exit=0) with 0 missing-location validator failures, while a raw `bun run build` at the same DB state reported 124 false-positives. Until the validator-renderer cascade divergence is closed (separate Planner ticket), treat raw `bun run build` as unsupported for any deploy decision.

**`BATCH_TIMEOUT=900` propagated to `1200` across 8 plists** (2026-05-29). `auto-enrich.sh:51` default was raised from 900→1200 on 2026-05-08 (S110h) because of "consecutive cap-edge wall-clock kills (904s, 905s)." The plists in `~/Library/LaunchAgents/com.agentathens.*.plist` were never updated, so every scheduled run was forced back to 900s — and died at ~905s mid-generation, before the in-subprocess `save-batch.ts` ran. Today's 08:08 scheduled slot was the first under the new value and saved 5 events (1 batch OK, 1 batch wall-clocked at 1214s — the 5-event right tail is creeping toward the new 1200s fence; if it persists, `EVENTS_PER_BATCH=4` is the next single knob). **Plist `.s27bak` backups present per plist for revert.**

**Cross-references:** `patterns.md` "Dual build paths diverge silently"; `mistakes.md` "Escalating 'fix the validator' without verifying which build path"; `specs/pipeline-sigterm-diagnostic-2026-05-27.md`; `specs/dual-build-path-divergence-2026-05-29.md`.

## S165 (refutation, genre-emission-gate) — Brief premise refuted at Step 0; no code change (2026-05-29)

**Decision:** Close S165 (the genre-emission-gate brief, distinct from the same-labeled image-finder and athinorama upgrader) as report-only refutation. Ship spec + notes; no code, no commit, no deploy. Brief prescribed a validator FAIL on empty `genre`/`about` + emission-site gate + S164 default removal under the "omit beats empty" ruling. Step 0 (read-only) refuted the premise on four counts: (a) 0 empty `genre`/`about` emissions in `dist/` — the brief's own success criterion already satisfied; (b) `genre` appears 882× in `/api/*.json` (flat data feed, non-Schema.org), 0× in HTML/JSON-LD — Event detail nodes emit no `genre` at all, so there are no emit sites producing `[]` to gate; (c) validator is warning-only (`src/generate-site.ts:1222`: "never blocks build"), so the brief's "Step 1 activation will fail build (intentional)" rests on a false wiring model; (d) S164 `visual-arts` confirmed as type-level default (`scripts/scrape-onassis.ts:201`) but feeds source `genres[]`, never emitted, and is documented in-code as a deliberate decision pending GEO Strategist sign-off.

**Refuted-brief naming convention adopted.** Refuted briefs keep the original S-number with a `-refutation` suffix (`specs/genre-emission-gate-S165-refutation.md`). Numbers are not burned. Rationale: burning creates index gaps that read as missing sessions; suffixes preserve the linear ledger and make refutations searchable as a class (`grep -l 'S.*-refutation' specs/`). Applies to all future refutations. **Pre-brief checklist gains an item:** planner greps `specs/` for the proposed S-number before issuing a brief (S165 has now been bound 3 times — image-finder refuted, athinorama upgrader shipped, genre-emission refuted — and the collision surfaced executor-side each time).

**Queued (not shipped this session):** (a) Removing S164 `['visual-arts']` default needs GEO Strategist ruling on whether "omit beats empty" extends to the source-field / API substrate (the ruling addressed JSON-LD emission only). (b) Promoting `schema-completeness.ts` from advisory to gate-mode is its own brief — every existing FAIL becomes a deployment blocker overnight; needs ratchet design parallel to the `sameAsSeverity` precedent. (c) **Option 4 (the inverse opportunity):** wire `genre` into Event JSON-LD with omit-on-absence. The classifier already computes `genreHint` (`src/categorizer/categorize-event.ts:48,261`) but no caller reads it. Source `genres[]` is populated and 882 values live in the API feed. Needs GEO Strategist scope spec covering per-EventType field mapping, classifier-confidence absence policy, and target-surface scope.

**Cross-references:** `specs/genre-emission-gate-S165-refutation.md`; `specs/image-finder-S165.md` (prior S165 refutation, label collision); `mistakes.md` / `patterns.md` S165 (refutation); user-memory `feedback_verify_paths_in_briefs`, `feedback_gate_gap_is_open_before_how_to_close`.

## S100b (GA4 leg) — A-vs-B outcome + GA4 field mapping + idempotency (2026-06-03)

**Decision: Scenario A (pure-Bun) shipped.** `@google-analytics/data@6.1.0` runs in-process under Bun's **default gRPC** transport (Step-1 probe, property 525325167, first attempt, no REST fallback needed). B-pivot (Python→JSON→Bun) not built — and its premise was stale anyway (no live `src/scraping/` Python convention; only `scripts/_archive/`). The importer does auth + query + filter + write entirely in Bun against the live client.

**GA4 field mapping (confirmed against live response, positional):** request dims `date`(YYYYMMDD) / `sessionSource` / `landingPage`, metrics `sessions`(0) / `activeUsers`(1) → table `ga4_ai_referrals(observed_date, referrer_engine, landing_page, sessions, imported_at)`. `ga4DateToIso("20260526")="2026-05-26"`. Metric stored is `sessions` (not `activeUsers`). Real `sessionSource` for ChatGPT = `"chatgpt.com"` (27/28d).

**Scope held at 3 hosts, NOT the 5 the CHECK allows.** `ga4_ai_referrals` CHECK permits `chatgpt/perplexity/gemini/copilot/claude`, but per the 2026-05-30 GEO ruling (AI-referral secondary metric only) the host→engine map carries exactly `chatgpt.com→chatgpt`, `perplexity.ai→perplexity`, `copilot.microsoft.com→copilot`. The schema is intentionally left wider than current scope (gemini/claude reserved for a future ruling); the importer does not widen to fill it. Discrepancy recorded for the operator rather than silently resolved either way.

**Idempotency: DB-enforced grain (operator decision, overrides brief's literal `INSERT OR REPLACE`).** Added `idx_ga4_airef_grain UNIQUE(observed_date, referrer_engine, landing_page)` to `kpi-init.ts` (applied to live db, ga4 table was empty so no violation risk) + `ON CONFLICT … DO UPDATE SET sessions=excluded.sessions, imported_at=excluded.imported_at`. Chosen over `INSERT OR REPLACE` because UPSERT updates in place (stable `id`, no delete-reinsert) and over DELETE-then-INSERT because the schema-level constraint is the durable guarantee against parallel-session double-writes. Live verify: re-run 18→18 rows, single `imported_at`.
...
## Session 180 — QID provenance applied to performer surface (2026-06-05)

- **Resolver-only generator:** `scripts/lookup-performer-sameAs.ts` SPARQL first-label-match engine REMOVED (it was corruption vector 1 — weak disambiguation); discovery now runs through `scripts/lib/performer-qid-resolver.ts` (Wikipedia wikibase_item pageprops anchor, else uniqueness-gated exact-label match, label-vs-owner verified). One codepath for generator, re-derivation, and periodic re-verify — no dual-path drift.
- **Any-language Wikipedia anchor (operator-ratified):** the wikibase_item binding is language-independent; en-only manufactures a fake underivable class. Label comparison is language-matched (el labels for Greek names). Multi-edition `sameAs` emission policy (when BOTH en+el articles exist) → **GEO ratification item, open**; current data has no contention (non-en anchors only where no en article exists).
- **`wikidata-label` is a verified method iff matchCount=1** (+ P106 occupation gate for humans): "no Wikipedia anchor → no QID" was a proxy; the principle is verified binding. Ambiguity = absence. Spec amended in place (specs/performer-sameas-audit-S177.md).
- **Operator-forced drops:** Μίνως Μάτσας (grandfather/grandson homonym — real-world ambiguity, no resolver outcome trustworthy), Chevy (ptwiki band ownership of Athens billing unconfirmed). Re-add only with human confirmation, via resolver.
- **Cache + manifest move as a pair** — both written only by resolver scripts; the build gate (`src/validators/performer-qid-manifest.ts`, wired pre-generation in generate-site.ts, exit 1) enforces it. Final tally: 23 confirmed / 24 corrected / 3 added / 4 dropped / 2 forced; 27/52 had been fabricated.

## Session 180 (post-session) — Three derivation policies, gated on the S179 binding rule

All three gate on the S179 rule: QIDs enter config only via resolver output; audits verify label-vs-owner, not QID-exists.

1. **Any-language anchor.** Resolver accepts any-language Wikipedia sitelink (el, de, …) as the wikibase_item pageprops derivation anchor, not en-only. A QID is language-independent; el and en anchors resolve to the same QID and are equally checkable by label-vs-owner. Label comparison must be language-matched or el-derived entries false-negative and silently drop.
2. **Uniqueness-gated label fallback.** When no Wikipedia anchor exists, a Wikidata entity may be accepted via exact label match + P106/P31 occupation gate (human/musician) ONLY if exactly one entity matches (match_count=1). Multiple matches → ambiguous → treated as absence → drop. Validator re-confirms uniqueness at validation time, not just QID-stability, so a future label-sharing entity can't silently make a clean entry ambiguous. (Implementation: the offline build gate enforces match_count=1 on record shape; uniqueness re-confirmation runs online via `scripts/audit-performer-qids.ts` re-runs — the deliberate validation pass for online-only facts, see patterns.md S180.)
...
## S104 (Session 183) — Validity metric: "structural error" ≡ native failCount; live guard is a subset of its claim (Outcome B) (2026-06-09)

**GEO definitional ruling (locked).** "Structural error" ≡ schema-completeness **native `failCount`** (FAIL-tier: `errors.length > 0` per page from `validateAllPages`). `data/build-completeness.json` and the completeness-reporter buckets are a **forbidden re-derived source** for the validity claim — NOT a second definition. The reporter's `classify()` shares the same `errors`-based rule, but per-bucket counts lag/drift across builds and re-aggregating them re-imports the 87/100 fold. Any validity copy sources from in-memory `schemaResults`, never the JSON.

**Verification verdict = B (invariant ⊂ claim).** The live "zero structural errors" guard is NOT `failCount == 0`. It is a **detail-page-only, `location`-substring hard-stop** (`generate-site.ts:1312`), alongside the hreflang and canonical-parity gates. FAILs on non-`isDetailPage` pages — hub `CollectionPage.itemListElement` nodes, plus the rest of the FAIL catalogue (`offers`/`startDate`/`FAQPage`/`streetAddress`) on any non-detail page — **ship ungated**. **C ruled out:** schema-completeness catches every fail natively (no validator-scope/emission gap, unlike the microdata incident). **A ruled out:** failCount was non-zero, and even at zero the claim is unguaranteed.

**Principles (bank → patterns.md).** (1) A citable invariant must be **coextensive with the claim it backs**; a guard covering a subset of its claim's scope passes green while the claim is false. (2) A "current-build" claim cannot be spot-checked against a **frozen/stale** production artifact — the freeze hides the very thing being verified. (3) **Node-trace is the only backfill-scope authority** — scope from the native FAIL node-trace (detail slug + hub `itemListElement` index → venue), never the reporter buckets (whose hubs.fail drifted 2↔4 across two reads of the same artifact while the native trace held steady).

**Sequencing (locked).** (1) streetAddress backfill = bleed-stop to unfreeze the daily deploy (frozen 2026-06-08 on detail-page drift; the hard-stop working as designed). (2) Stage-1 = promote the guard to **all-class `failCount == 0`** (every FAIL-tier rule, every page class) — GEO's decision, authored ONLY after the backfill verifies clean; it would currently FAIL the build on the ungated hub nodes (correctness-guarantee bought with deploy-cadence resilience). (3) **Colophon validity wiring is downstream of Stage-1** — "0 structural errors" becomes citable *by construction* once the all-class invariant exists, not before. Closes the open build-ordering question left in the S103 (Session 181) entry.

**Cross-references:** `patterns.md` S104; `src/validators/schema-completeness.ts` (`validateAllPages`, FAIL catalogue), `src/validators/completeness-reporter.ts` (`classify`), `src/generate-site.ts` (1268 validate, 1312 detail-location hard-stop), `src/utils/schema-graph-builders.ts:63` (config-`address` → `streetAddress` cascade); `specs/streetaddress-backfill-brief.md`.

## Location verification (S182)

| Decision | Why | Date |
|----------|-----|------|
| Config-vs-code routing outcome: **mechanical subset empty (0/201) → no config change, route entire 201 to Editorial whitelist-curation** | S182's Method-1 (promote venues whose decoded/normalized name matches an *existing* whitelist entry) was computed against the real matcher as 0 events. No `athens-venues.json` change, no DB mutation, no test — there was nothing mechanical to fix. The 201 are absent from the whitelist, so promoting them is a venue-Athens judgment (Method 2/3), not normalization. | 2026-06-09 (S182) |
| Defer accent-folding `normalize()` (D1) to planner as a code-path decision | Accent-blind `normalize()` lets other-city ALL-CAPS venues (`ΗΡΑΚΛΕΙΟ`) escape the blacklist into `unverified`. Fixing it is new shared match-logic code touching every match site (Guard 6) and only *rejects* noise (promotes 0 Athens events) — an architecture call, not an in-session mechanical fix. Proposed shape recorded in `specs/location-verification-residual-S182.md` §2. | 2026-06-09 (S182) |
| Do NOT re-add entity variations to the whitelist (D2); prefer decode-at-scrape | `decodeEventFields` (S154) is the chosen durable path and its module deprecates whitelist entity-variations. Re-adding them for 5 stale-row venues would fight that direction; let re-scrape clear them. | 2026-06-09 (S182) |

**Open for planner (not decided here):** (1) D1 accent-fold normalizer; (2) D2 stale-entity re-decode/migration; (3) the Editorial whitelist/blacklist pass over the 105 residual venues. The "201 stranded backlog" is **skewed toward other-city noise** (D1), not stranded Athens events — a minority (Herodion, Δόρα Στράτου, Θέατρο Βράχων, Ζάππειο, Πεδίον του Άρεως, Βοτανικός Κήπος) are genuinely Athens.
...
| Decision | Rationale | Date |
|----------|-----------|------|
| noindex the dormant bare-roots; do NOT re-include in sitemap | The page is a dormant Greek alternate while the flip gate is closed; the indexable representative is `/en/`. noindex+absent is the consistent dormant state. Re-inclusion would reverse the deliberate S144 GEO ruling. | 2026-06-09 (S184) |
| noindex driven by the runtime sitemap-drop predicate, NOT the static `answerCapsuleEn` flag | The class is **dynamic**: a hub with `answerCapsuleEn` but a low-event day emits no `/en/` twin and stays in the sitemap, so it must stay indexable (`/tomorrow`, `/dance` proved this live). noindex must read the SAME test that drops it (`bilingualHubSlugs` / `BILINGUAL_CONTENT_SLUGS`), gated by `!HREFLANG_GATE_OPEN`. | 2026-06-09 (S184) |
| Build-FAIL invariant is output-keyed + bidirectional, liveness from the sitemap not disk | Rule 1 (in-sitemap⟹indexable) + Rule 2 (live twin⟹noindex∧absent), over emitted artifacts — immune to the hardcoded-`HUB_SLUGS` rot (which had lost tomorrow/this-week/next-month). "Live twin" = `en/<slug>/` in a freshly-regenerated sitemap, NOT `dist/en/<slug>/` existence (a stale `en/dance` artifact would false-FAIL). Standalone pass (mirrors `validateDistCanonicalParity`), not `validateAllPages` (per-page-HTML-only, can't see sitemaps). | 2026-06-09 (S184) |
| Fix `/saved/` inverse-drift in the same change (class 22 → 23 surfaces) | `/saved/` was `noindex` AND in the sitemap — advertising a noindex URL for crawl, the inverse of the `/today` defect, surfaced by Rule 1. Excluded from sitemaps (same policy as `/all/` overflow). | 2026-06-09 (S184) |

**Amendment back to GEO (ruling stands):** the class is **22 pages, not 24** (−proof/colophon/saved already noindex, +tomorrow), and "drift = dropped-AND-indexable, runtime-keyed"; the invariant catches **both** drift directions (the `/saved/` inverse was the +1 → 23 surfaces touched). The 22-vs-24 is a live-enumeration correction, not a re-decision.
...
| Decision | Why | Date |
|----------|-----|------|
| `resolveEffectiveEnd()` in event-lifecycle.ts is the ONLY derivation of "when does an event effectively end" | The exhibition-only special case had metastasized into 20+ hardcoded `type==='exhibition'` sites (A2 F2/F3 root). Status, lifecycle, phases all consume it; consumers that re-derive locally are the next F2. | 2026-06-13 (S189) |
| Presumption expiry → cooling immediately, skipping just-passed | just-passed is "we KNOW it recently ended" (indexed 14d). A presumed end is not knowledge — err short per G1. | 2026-06-13 (S189) |
| G1 invariant (b) is a structural throw at emission, not a validator check | HTML-level validation can't see DB-null end_date; making synthesized-endDate-on-NULL-end unconstructable is stronger than detecting it. Deviation from brief documented in specs/f2b-residual.md. | 2026-06-13 (S189) |
| Schema validator failCount>0 now exits 1 (armed in generate-site.ts) | A2 F1 deployed 5 FAILs for weeks because this layer was warning-only. Armed at fail=0 baseline. Any future FAIL = blocked deploy BY DESIGN. | 2026-06-13 (S189) |
| Sitemap membership and robots-noindex read the SAME `shouldNoindexEvent` predicate, both locales | G3 single-state-machine guard; parallel flags are how the 422-page drift happened. | 2026-06-13 (S189) |
| Ticket-CTA past-check uses real end_date for any type, but NEVER presumption | Sellability is a fact claim; presumption affects status/indexing only. | 2026-06-13 (S189) |
...
### Backup integrity floors + DB-missing breach (2026-06-30)
...
### DOC DEBT: design-system.md §4 grid table is STALE — do not cite (2026-07-05)
`docs/design-system.md` §4 (grid) says 3-col desktop / 32px gutter / 2-col tablet. Production has DIVERGED: `.date-group` is **4-col ≥1280 / 3-col 1025–1279 / 2-col ≤1024 / 1-col ≤767**, gutter `--space-md-lg` = **24px**. The "~408px" prose is also stale. A downstream ruling read §4 as truth and shipped a fixed-px orphan cap (the 146px squeeze bug). **Do NOT cite §4 for grid/gutter values until reconciled — read design-system.css.** Full §4 rewrite joins the doc-reconciliation pass.

| Dedup losers get `merged_into` column (migration 013), never DELETE; loser pages keep emitting until GEO ruling | Reversible with one UPDATE; a new nullable column is invisible to every existing query, so emission/enrichment behavior is provably unchanged (a status-value change would leak into WHERE clauses). Disposition of 27 live loser URLs = GEO Strategist call: specs/dedup-url-disposition-proposal.md | 2026-07-05 |
| `findDuplicateGroups` (union-find, all-pairs) added BESIDE `findDuplicates` (first-match pairs) instead of replacing it | Daily merge script consumes pair semantics incl. the matched-set guard; changing it mid-arc risks the 09:34 cron. Group semantics only for the new mark pass + import gate. Daily swap to mark-based merging is an explicitly separate session | 2026-07-05 |
| Survivor policy: enrichment tier > richness score > HOLD (no updated_at tie-break) | Brief routes genuine ties to human review (specs/dedup-hold.md) rather than pseudo-deterministic timestamps. 0 ties occurred in the 25-group retroactive pass | 2026-07-05 |
| Import gate (upsertEvent, new ids only) fails OPEN and reuses the exact merge-pass matcher | A gate crash must never block event collection (address-guard precedent); one matcher at both pipeline ends means import-time behavior is testable against merge-pass fixtures. Same-id re-scrapes keep the UPDATE path untouched | 2026-07-05 |

| Mark-based dedup (`mark-duplicates.ts`) is now the CANONICAL daily cross-source path; `merge-duplicates.ts` (DELETE-based) disconnected from automation but left on disk | Reversibility: losers marked `merged_into` are undoable; field-merge benefit preserved (mark-duplicates still merges best-of fields onto survivor). Same-source `remove-duplicates.ts` stays wired (different behavior) — it's `merged_into`-guarded so it can't delete a marked loser. No `--exclude-layers`: Layer 4 un-shadowed (safe under reversible marker; 5 of 27 marks at daily scale, 0 false merges in S197 self-gate) | 2026-07-05 (S198) |
| Merged losers excluded from enrichment SELECTION only (not acquisition/reporting) | The waste the S197 report named was Claude description budget (`needs_enrichment` queue) — 20 of 27 losers were queued. Guarded the 3 live selection owners. Left price/ticket/geo/image/schema/health `location_status IN` sites untouched: they keep the still-building loser pages valid until the GEO ruling, and are different behaviors (Guard-6-on-owners-not-terms) | 2026-07-05 (S198) |
| Pageless-survivor groups re-pointed to the emitting row (pointer columns only, no field re-merge) | A survivor of record must be a page that exists; richness-based selection had picked A0-hard-stop-excluded rows. Boundary kept to `merged_into`/`merged_at` — field re-merge onto the new (thinner) survivors is an optional follow-up; `dedup_merges` audit retains original direction (live truth is `events.merged_into`) | 2026-07-05 (S198) |

## 2026-07-07 — Visibility campaign: cycles in dedup mark-state FAIL OPEN per-group; dangling stays build-FAIL; foreign strand left uncommitted; no manual deploy
**Context:** dedup-301 emission (uncommitted strand implementing "GEO Ruling 2026-07-05, Option 2") froze the Jul 6 daily build on a mutual/flipped mark pair; freshness term zeroed since Jul 4 (stacked with the Jul 5 streetAddress freeze, fixed at b76ad0ef8).
**Decisions (executor, dev-plumbing scope):**
1. **Chains resolve to terminal** — implements the ruling's own "terminal survivor" wording; not a policy change.
2. **Cycles (no terminal exists) fail OPEN for that group only:** both pages emit (status quo ante), no rule, loud `⚠️ dedup-301` warn. Rationale: the ruling's invariant ("no 301 to a non-emitting target") is preserved vacuously; freezing the whole site on transient marker corruption zeroes freshness — the binding citability constraint. GEO may re-rule this disposition (flagged in specs/visibility-ceiling.md).
