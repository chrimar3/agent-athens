...
describe('externalScriptSrcIssue / scanHtmlForArtifacts', () => {
  ...
  for (const [what, src] of BAD) {
    test(`fails with a clear message: ${what}`, () => {
      ...
      // Exactly one pin finding (a data: URL also trips the unsafe-scheme check).
      expect(issues.filter((i) => i.includes('not an allowed script URL'))).toHaveLength(1);
    });
  }
  ...
  // The CLI ships with the protected-paths PR; the test runs once both are merged.
  test.skipIf(!existsSync(join(ROOT, 'scripts/check-published-artifacts.ts')))('the standalone CLI (publish mode re-check) fails a dist/ with a pinned-out script, naming the page', () => {
