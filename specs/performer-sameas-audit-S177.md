...
1. **Deterministic re-derivation, no lookup model:** for each entry, derive the QID from the entry's bound Wikipedia article via `<lang>.wikipedia.org/w/api.php?action=query&prop=pageprops&ppprop=wikibase_item&titles=<title>` — Wikipedia returns the authoritative QID. The anchor may be **any-language** Wikipedia sitelink (el, de, …): the wikibase_item binding is language-independent, and label-vs-owner comparison is **language-matched to the anchor edition** (compare against el labels for Greek names, or el-derived entries false-negative and silently drop). Where no Wikipedia article exists in any edition, a QID may still be accepted via the **uniqueness-gated Wikidata label method**: exact normalized label + P106/P31 occupation gate (human/musician) + match_count=1. Ambiguous / multi-match → drop per absence-beats-fabrication.
   **[S180 amendment trail, operator-ratified]** The paragraph above replaces two original S177 rules — en.wikipedia-only anchors, and an absolute anchorless-QID drop. Both were spec under-specification of the resolver, not principle (en-only manufactured a fake underivable class: 5 Greek artists + Marika Rossa anchor via el/de; the anchorless drop would have discarded 5 correctly-bound label-verified entities). The occupation gate exists because exact label + uniqueness alone let an ancient diplomat through for "Leon of Athens". Multi-edition sameAs emission policy (en vs el when both exist) is a GEO ratification item, non-blocking — current batch has no contention (non-en anchors exist only where en doesn't).
