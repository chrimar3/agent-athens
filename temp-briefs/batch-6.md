...
- This is Batch 6
- Event IDs: 7b9483036af8290f, ab8c5589dc5e9ad3, c80a0830d3338063
- Write descriptions to: temp-descriptions/batch-6/
...
- **Time**: 22:00
- **URL**: https://ra.co/events/2389815
...
2. **Write description**: Save to batch directory (temp-descriptions/batch-6/):
   ```bash
   bun run scripts/write-description.ts <event-id> --batch-dir=temp-descriptions/batch-6 "<description text>"
   ```
3. **Gate check**: Validate quality with metadata flags (no DB needed):
   ```bash
   bun run scripts/auto-gate-check.ts temp-descriptions/batch-6/<event-id>.md --tier=<tier> --event-id=<event-id> \
...
4. **Write tags** (from taxonomy in docs/MASTER-ENRICHMENT-TEMPLATE.md):
   ```bash
   bun run scripts/write-tags.ts <event-id> --batch-dir=temp-descriptions/batch-6 Tag1 Tag2 Tag3...
   ```
5. **Write Greek description** (optional secondary): Write a condensed Greek version.
   Save to `temp-descriptions/batch-6/<event-id>.gr.md`. Greek word target shown per event above.
   ```bash
   bun run scripts/write-description.ts <event-id> --batch-dir=temp-descriptions/batch-6 --lang=gr "<greek description>"
   ```
...
```bash
bun run scripts/auto-gate-check.ts temp-descriptions/batch-6/7b9483036af8290f.md \
  --tier=stub --event-id=7b9483036af8290f \
  --event-type=dj_set --event-venue="Burger Disco Club" \
  --event-title="ClubKid x Nikolas Gale" \
  --event-date=2026-03-13 --event-price=paid
```

```bash
bun run scripts/auto-gate-check.ts temp-descriptions/batch-6/ab8c5589dc5e9ad3.md \
  --tier=standard --event-id=ab8c5589dc5e9ad3 \
...
```bash
bun run scripts/auto-gate-check.ts temp-descriptions/batch-6/c80a0830d3338063.md \
  --tier=standard --event-id=c80a0830d3338063 \
  --event-type=theater --event-venue="Ακροπόλ" \
  --event-title="Όταν ο Μίκης ήταν παιδί" \
  --event-date=2026-03-13 --event-price=paid
```

After all events, create `temp-descriptions/batch-6/batch-6-review.md` with:

| Event ID | Title | Gate Score | Issues | Confidence |
|----------|-------|------------|--------|------------|
| 7b9483036af8290f | ClubKid x Nikolas Gale | /100 | | |
| ab8c5589dc5e9ad3 | Δεν θα πεθάνουμε κιόλας | /100 | | |
| c80a0830d3338063 | Όταν ο Μίκης ήταν παιδί | /100 | | |
